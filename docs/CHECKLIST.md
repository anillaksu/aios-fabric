...
- [ ] **M-9 (owner katkısı, 2026-08-18)** **Depolama yönü ters çevrilsin: sunucu birincil, istemci önbellek.** Bugün (W6.F) `IndexedDB` istemci tarafında birincil kaynak, sunucudaki `/artifacts` yalnızca tek yönlü yedek (istemci→sunucu, geri okumuyor). Bu ters — Fabric zaten Termux'un kendi süreci, sınırsız erişimli dosya sistemi var; tarayıcının kısıtlı/denetlenmesi zor deposunu (bkz. bugünkü SELinux/Shizuku/root araştırması, `persist()` hâlâ TARGET) birincil yapmak gereksiz risk. **Yön:** `/artifacts` GET'i açılışta gerçekten okunsun (bugün yalnızca DevTools ekranı için kullanılıyor), istemci onunla senkronlansın; `IndexedDB` yalnızca çevrimdışı/hızlı-açılış önbelleği olsun. Kapsam: yalnızca depolama katmanı — Fabric'in kendisi Termux'ta kalır, Ubuntu'ya taşınmaz (bkz. B-11, taşımanın 18+ capability'yi bozma riski var)
