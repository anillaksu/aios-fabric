...
- [~] W6.B **Boş kanvas + pencere yöneticisi.** **DÜZELTME (2026-08-17, `PLAN_W6_app-shell.md §W6.2`):** madde ilk yazıldığında "çok pencereli, sürüklenebilir, z-index yönetimi" diyordu — bu, KARAR-1'in **reddettiği** masaüstü modeliydi. Düzeltilmiş kapsam: sabit **ızgara** (Android widget gibi, çakışma yok) + dokununca **odaklı tam ekrana** açılma; `WindowManager` çekirdeği (open/close/focus/persist) yüzeyden ayrık yazılır — sürükleme/boyutlandırma/z-index yığını **yok**
      ...
      · **Güncelleme (2026-08-17, aynı gün):** artık bir yüzeye bağlı — bkz. W6.C. `focus/unfocus` gerçek artefakt açma/kapamada kullanılıyor. Kalan: ızgara yerleşiminin kendisi (bugün hâlâ liste/kart görünümü, KARAR-1'in ızgarası değil) — bu W6.C'nin ikinci diliminde ele alınacak
- [~] W6.C **Boş kanvas enjeksiyonu.** Kabuk boş bir kutu açar, Hermes'e yalnızca "bu kutunun içi" için üretim yaptırılır
      · **İlk dilim yapıldı (2026-08-17):** W6.B'nin yüzeyden bağımsız `WindowManager`'ı **gerçek yüzeye** bağlandı — `openArtifact(id)` artık `wm.register()` + `wm.focus()` çağırıyor, geri dönüş `wm.unfocus()` çağırıyor. **Yeni üretim yok** — var olan `artifactBlock()`/`render()` aynen kullanıldı (plandaki "Katman A içeriği doğrudan mount edilir, yeni kod yok" kuralına uygun). Izgara→tam ekran geçişi `document.startViewTransition` ile (özellik algılama, tarayıcı algılama değil)
      · **TEST-VERIFIED** — `BUILD_OK` + 21/21 test (telefonda da), md5 birebir (38 dosya), telefonda **4 gerçek artefakt** var (sentetik veri kullanılmadı). Kod yolu küçük (15 satır) ve gözden geçirildi
      · **Kalan iş — FACT'e çıkması için:** telefonda bir artefakt açılıp kapatılarak görsel regresyon yok kanıtlanmalı (K5, görünür çıktı **bilerek değişmedi**, bu yüzden "yeni bir şey görünüyor" değil "hiçbir şey bozulmadı" kanıtı gerekiyor)
