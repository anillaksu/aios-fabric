...
- [~] W6.F **Kalıcı galeri + önbellek.** Üretilen widget `IndexedDB`'ye yazılır; tekrar açılışta **yapay zekaya sorulmaz**, sıfır gecikme/sıfır maliyet
      ...
      · **`navigator.storage.persist()` çalışma zamanı sonucu (`true`/`false`) doğrulanamadı — üç bağımsız yoldan denenip tükendiği kayda geçti (2026-08-17):** (1) Shizuku/`rish` ile IndexedDB'yi doğrudan incelemeye çalışıldı → *"Server is not running"*; (2) `adb devices` → boş liste; (3) Android'in kablosuz ADB portunu yayınladığı iki sistem property'si de (`service.adb.tcp.port`, `service.adb.tls.port`) **boş**. Üçü de aynı kök nedene işaret ediyor: **Kablosuz Hata Ayıklama telefonda kapalı** — bu yalnızca cihazdan elle açılabilir (Termux'un `uid=10322` sandbox'ı `WRITE_SECURE_SETTINGS` iznine sahip değil, `02-shizuku.sh` bunu zaten belgeliyor). mDNS ile port keşfi de denendi, `adb`'nin bu derlemesinde mDNS alt sistemi çalışmıyor (`unknown host service`) — though asıl neden zaten port'un hiç açılmamış olması. **Kod var, çalışma zamanı sonucu yalnızca anahtar açılırsa görülebilir** → TARGET, ama artık "denenmedi" değil "denendi ve neden şu an imkânsız olduğu kanıtlandı"
