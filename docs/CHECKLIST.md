...
- [ ] B-10 **(2026-08-17'de bulundu, W6.C'nin "boş pencere" canlı testinde owner bildirdi)** Uygulama tamamen kapatılıp yeniden açıldığında **soğuk başlangıçta `#tabbar` görünmüyor** — yazma kutusuna dokunup klavye açtırıp kapatınca (bir `visualViewport` resize/scroll olayı tetiklenince) düzeliyor. Kök neden **doğrulanmadı**: `aios.html`'in `visualViewport` senkron kodu (`sync()` ilk kez script yüklenirken bir kez çağrılıyor) bu oturumda **hiç değiştirilmedi** — muhtemelen önceden var olan bir zamanlama yarışı (sayfa tam yerleşmeden `vv.height`/`innerHeight` yanlış okunup `body.kb` sınıfı hatalı takılıyor, `body.kb #tabbar{display:none}` kuralı devreye giriyor). W6.I'nin sayfa yükünü küçültmesi (1.444→150 KB) zamanlamayı değiştirip **gizli bir hatayı ortaya çıkarmış olabilir** — ama bu **doğrulanmadı**, spekülasyon olarak işaretlendi. Ayrı bir oturumda cihazda (gerekirse Chrome DevTools remote debug ile) araştırılmalı
