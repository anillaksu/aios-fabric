...
- [ ] B-8 **(2026-08-17'de bulundu, 2026-08-18'de genellendi)** `deploy-to-phone.sh` yalnızca **kopyalıyor**, depodan **silinen** dosyaları telefondan kaldırmıyor. İlk örnek: Framework7 bundle'ları (808+486 KB) depodan silindi ama telefonun diskinde kaldı, `vendor/` md5 kapsamı dışı (B-4) olduğu için doğrulama da yakalamadı. **İkinci örnek (B-3 dağıtımında yakalandı):** `js/components.css` silinince md5 doğrulaması bu kez **doğru şekilde patladı** (`vendor/` kapsamında değildi) — elle `rm` ile düzeltildi. İkisi de aynı kök nedene işaret ediyor: script'e bir "yerelde yok ama telefonda var → sil" adımı gerekiyor, `rsync --delete` benzeri
