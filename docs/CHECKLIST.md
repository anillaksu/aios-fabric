...
Son güncelleme: 2026-08-18 (W6.K kapandı) · Kanonik depo: `C:\Users\anil\Desktop\aios-fabric`
...
- [ ] B-9 **(2026-08-17'de bulundu, W6.C doğrulaması sırasında)** Android arka planda **tüm Termux oturumunu** (`llm_bridge` + `watchdog.sh` + eski `fabric`) öldürebiliyor — **2026-08-17'de iki kez** (`gateway.log`: *"exited UNCLEANLY — SIGKILL/OOM/VM death"*), **2026-08-18'de üçüncü kez** (SSH portu 8022 tamamen kapandı — `sshd` dahil tüm Termux gitmiş; sonra ayrıca yalnızca `llm_bridge` tekrar öldü, `task.failed: "fetch failed"` journal'da görüldü, port 9201 kapalıydı) — **toplam üç kez, tek oturumda**. `watchdog.sh` kendisi de aynı anda ölüyor, kendini yeniden başlatamıyor — gözcünün gözcüsü yok. `termux-wake-lock` yeterli gelmiyor. Kalıcı çözüm yok; `Termux:Boot` / foreground service / daha agresif pil istisnası araştırılmalı. **Etki büyüyor:** W6.L'nin ilk iki canlı test denemesi bu yüzden yanlış teşhis edildi (önbellek sorunu sanıldı, gerçekte backend ölmüştü) — B-9 artık yalnızca operasyonel değil, **doğrulama sürecini bulandıran** bir risk
