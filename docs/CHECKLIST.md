...
- [~] W6.L **Prompt→kod önbelleği.** SHA-256 (`crypto.subtle`) ile eşleme; normalizasyon zorunlu, hash'e capability sürümü katılır.
      ...
      · **Kalan iş — FACT'e çıkması için:** istemci tarafı (`getCached`/`putCached`, IndexedDB) yalnızca canlı cihazda doğrulanabilir — aynı isteğin ikinci kez **ağ isteği/model çağrısı olmadan** geldiğinin görülmesi gerekiyor (K5)
