...
$SSH "$PHONE" 'D=~/backup-$(date +%Y%m%d-%H%M%S); mkdir -p $D/css $D/test $D/shortcuts $D/boot && cp ~/fabric/src/*.ts ~/fabric/package.json ~/fabric/public/js/* ~/fabric/public/aios.html ~/fabric/public/sw.js ~/fabric/public/manifest.json ~/start_hermes_os.sh ~/watchdog.sh $D/ 2>/dev/null; cp ~/.shortcuts/start_hermes_os.sh ~/.shortcuts/provision-fabric-gateway-key.sh ~/.shortcuts/aios-admin-console.sh ~/.shortcuts/aios-workspace-map.sh ~/.shortcuts/aios-connectivity-bridge.sh $D/shortcuts/ 2>/dev/null; cp ~/.termux/boot/10-aios.sh $D/boot/ 2>/dev/null; cp ~/fabric/public/css/* $D/css/ 2>/dev/null; cp ~/fabric/test/*.test.ts $D/test/ 2>/dev/null; echo "$D ($(find $D -type f | wc -l) dosya)"'
...
$SCP "$REPO"/scripts/start_hermes_os.sh "$REPO"/scripts/watchdog.sh "$REPO"/scripts/10-aios.sh "$REPO"/scripts/provision-fabric-gateway-key.sh "$REPO"/scripts/aios-admin-console.sh "$REPO"/scripts/aios-workspace-map.sh "$REPO"/scripts/aios-connectivity-bridge.sh "$PHONE":'~/' || fail "kanonik baslaticilar kopyalanamadi"
$SSH "$PHONE" 'mkdir -p ~/.shortcuts ~/.termux/boot && cp ~/start_hermes_os.sh ~/.shortcuts/start_hermes_os.sh && cp ~/10-aios.sh ~/.termux/boot/10-aios.sh && cp ~/provision-fabric-gateway-key.sh ~/.shortcuts/provision-fabric-gateway-key.sh && cp ~/aios-admin-console.sh ~/.shortcuts/aios-admin-console.sh && cp ~/aios-workspace-map.sh ~/.shortcuts/aios-workspace-map.sh && cp ~/aios-connectivity-bridge.sh ~/.shortcuts/aios-connectivity-bridge.sh && chmod 700 ~/start_hermes_os.sh ~/watchdog.sh ~/.shortcuts/start_hermes_os.sh ~/.shortcuts/provision-fabric-gateway-key.sh ~/.shortcuts/aios-admin-console.sh ~/.shortcuts/aios-workspace-map.sh ~/.shortcuts/aios-connectivity-bridge.sh ~/.termux/boot/10-aios.sh' || fail "Widget/boot baslaticilari hazirlanamadi"
