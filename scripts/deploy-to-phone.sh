...
REL_PATHS="fabric/src/*.ts fabric/test/*.test.ts fabric/package.json fabric/public/js/* fabric/public/css/* fabric/public/aios.html fabric/public/sw.js fabric/public/manifest.json scripts/start_hermes_os.sh scripts/10-aios.sh scripts/watchdog.sh scripts/provision-fabric-gateway-key.sh"
...
$SCP "$REPO"/scripts/start_hermes_os.sh "$REPO"/scripts/watchdog.sh "$REPO"/scripts/10-aios.sh "$REPO"/scripts/provision-fabric-gateway-key.sh "$PHONE":'~/' || fail "kanonik baslaticilar kopyalanamadi"
$SSH "$PHONE" 'mkdir -p ~/.shortcuts ~/.termux/boot && cp ~/start_hermes_os.sh ~/.shortcuts/start_hermes_os.sh && cp ~/10-aios.sh ~/.termux/boot/10-aios.sh && cp ~/provision-fabric-gateway-key.sh ~/.shortcuts/provision-fabric-gateway-key.sh && chmod 700 ~/start_hermes_os.sh ~/watchdog.sh ~/.shortcuts/start_hermes_os.sh ~/.shortcuts/provision-fabric-gateway-key.sh ~/.termux/boot/10-aios.sh' || fail "Widget/boot baslaticilari hazirlanamadi"
