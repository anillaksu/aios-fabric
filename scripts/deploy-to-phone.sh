...
$SCP "$REPO"/scripts/start_hermes_os.sh "$REPO"/scripts/watchdog.sh "$REPO"/scripts/10-aios.sh "$REPO"/scripts/provision-fabric-gateway-key.sh "$REPO"/scripts/aios-admin-console.sh "$PHONE":'~/' || fail "kanonik baslaticilar kopyalanamadi"
$SSH "$PHONE" 'mkdir -p ~/.shortcuts ~/.termux/boot && cp ~/start_hermes_os.sh ~/.shortcuts/start_hermes_os.sh && cp ~/10-aios.sh ~/.termux/boot/10-aios.sh && cp ~/provision-fabric-gateway-key.sh ~/.shortcuts/provision-fabric-gateway-key.sh && cp ~/aios-admin-console.sh ~/.shortcuts/aios-admin-console.sh && chmod 700 ~/start_hermes_os.sh ~/watchdog.sh ~/.shortcuts/start_hermes_os.sh ~/.shortcuts/provision-fabric-gateway-key.sh ~/.shortcuts/aios-admin-console.sh ~/.termux/boot/10-aios.sh' || fail "Widget/boot baslaticilari hazirlanamadi"
